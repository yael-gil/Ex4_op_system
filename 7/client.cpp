/*
** client.cpp -- UDS stream client with algorithm selection
** שולח: בקשת אלגוריתם עם פרמטרים או מטריצת שכנויות
** מקבל: תוצאת האלגוריתם המבוקש
*/
...
// ---------- קריאת מטריצה מ-stdin ----------
std::string read_matrix_from_stdin() {
    std::string matrix;
    std::string line;
    
    std::cout << "Enter adjacency matrix (end with Ctrl+D):\n";
    std::cout << "Format: space-separated integers, one row per line\n";
...
// ---------- קריאת מטריצה מקובץ ----------
std::string read_matrix_from_file(const std::string& filename) {
...
int send_request_and_receive_response(const std::string& request) {
    ...
    for (;;) {
        ...
        struct timeval tv;
        tv.tv_sec = 10; // טיים-אאוט של 10 שניות
        tv.tv_usec = 0;
...
int main(int argc, char* argv[]) {
    ...
    if (use_file) {
        // קריאת מטריצה מקובץ
        std::string matrix = read_matrix_from_file(matrix_file);
        if (matrix.empty()) {
            return 1;
        }
    ...
    } else if (use_stdin || (V == 0 && E == 0)) {
        // קריאת מטריצה מ-stdin
        std::string matrix = read_matrix_from_stdin();
        if (matrix.empty()) {
            std::cerr << "Error: No matrix data provided\n";
            return 1;
        }
